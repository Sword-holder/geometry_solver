...
def create_problem():
    ...
    angle_def = Angle('DEF', sides=[line_de, line_ef], vertex=p_e, angle=None)
    angle_dfe = Angle('DFE', sides=[line_df, line_ef], vertex=p_f, angle=None)
    angle_edf = Angle('EDF', sides=[line_de, line_df], vertex=p_d, angle=None)
    ...
    r = SimilarTriangle('ABC and DEF', triangle1=triangle1, triangle2=triangle2, ratio=2, corresponding=[(angle_abc, angle_def), (angle_acb, angle_dfe), (angle_bac, angle_edf)])
...
def create_target(problem):
    target = Target(TargetType.EVALUATION,
                    entity=problem.entity.find_child('DF', Line),
                    attr='length')
    return target
...
def test_similar_triangle():
    ...
    assert problem.entity.find_child('DF', Line).length - 3**(1/2) < 0.00001
